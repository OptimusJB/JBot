...
func serveur_sauvegarder():
	...
	# on met à jour blacklist (en dehors de data comme ça c'est plus simple pour l'owner de gérer
	fichier = FileAccess.open(serveur_dossier_save + serveur_fichier_blacklist, FileAccess.WRITE)
	fichier.store_string("mettre ici les pseudos des utilisateurs shadow ban. 1 ligne = 1 pseudo")
	for pseudo in blacklist:
...
func serveur_charger():
	...
	# on récupère la blacklist
	blacklist = []
	fichier = FileAccess.open(serveur_dossier_save + serveur_fichier_blacklist, FileAccess.READ)
	contenu_fichier = fichier.get_as_text().split("\n")
	for index_element in range(len(contenu_fichier) - 1):	# le séparateur est \n
		blacklist.append(contenu_fichier[index_element] + 1)	# +1 pour éviter de récupérer les instructions du début du fichier
	fichier.close()
...
func serveur_auto_repare():
	...
	if not dossier.file_exists(serveur_fichier_blacklist):
		fichier = FileAccess.open(serveur_dossier_save + serveur_fichier_blacklist, FileAccess.WRITE)
		fichier.store_string("mettre ici les pseudos des utilisateurs shadow ban. 1 ligne = 1 pseudo")
		fichier.close()
		print("fichier blacklist créé car absent")
